...
library(knitr)
rm(list=ls())
setwd("/home/jrm/scm/tibisay_cephalopods.git/sims/sim16")
...
\begin{itemize}
\item Simulation under the null for branch-site model A \textcolor{red}{with 10,000 sites}
...
\item $\kappa=2.0$ $p_0=0.5$ $p_1=0.5$ $\omega_0=0$ $\omega_2=1$
\end{itemize}
