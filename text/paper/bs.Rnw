...
\usepackage{mathabx} % downward arrow
...
% need subfloat b/c fig.subcap in knitr was built with deprecated subfig package
%\newcommand{\subfloat}[2][need a sub-caption]{\subcaptionbox{#1}{#2}}
\usepackage[flushleft]{threeparttable} % description under table
\usepackage[noindentafter,tiny]{titlesec} % spacing around section headings
...
% page heading title
\pagestyle{myheadings}\markright{BSA}
...
library(grid)
library(knitr)
setwd("/home/jrm/scm/tibisay_cephalopods.git/text/paper")
...
\noindent \textbf{\large Unrecognized statistical difficulties with inference of positive selection under the branch-site family of codon models}
\vskip 4mm
\noindent {\small Joseph Mingrone,$^{*,1,2}$ Edward Susko,$^{1,2}$ Joseph Bielawski$^{1,2,3}$}\\
\textit{\scriptsize $^1$Department of Mathematics and Statistics,Dalhousie University, Halifax, NS, Canada\\
...
\textbf{*Corresponding Author:} E-mail: jrm@ftfl.ca}
...
\begin{abstract}
  \noindent Branch-Site models of codon evolution are widely used for detecting
  positive selection that has acted on specified lineages of a phylogeny.  Two
  different likelihood ratio tests have been described, each with thresholds for
  declaring positive selection determined from a chi-square or mixture of
  chi-square distributions.  A first test, which, under the null hypothesis, did
  not permit selection to vary across branches, was shown to flag positive
  selection with unacceptably high false positive rates.  A second test was
  developed that does not suffer from issues with inflated false positive rates.
  Here, we describe problems with this second test.  The probability of type I
  errors varies according to factors such as the branches considered for
  positive selection and irregularity with certain parameter estimates.  In some
  cases, the test is so lacking in power, that a re-calibration should be
  considered.  We also describe how confounding problems can occur when positive
  selection is identified in the wrong location in the phylogeny.
\end{abstract}

\noindent\textbf{Availability:} https://github.com/jehops/???\\
\textbf{Contact:} jrm@ftfl.ca\\
\textbf{Supplementary information:} Supplementary data are available at .

%\onehalfspacing
\doublespacing

\noindent Key words: positive selection, likelihood ratio test, molecular adaptation.
...
\subsection*{Extremely conservative LR distribution approximations}
Figure \ref{fig:CDFNull1FBTL6} shows estimated LR CDFs for Branch-Site Model A LRT2 simulations under the null model.  These data were simulated using the 8-taxon tree with branch lengths summing to $6$.  CDFs for $\chi_0^2/2 + \chi_1^2/2$ are also included in each plot.  For all of the CDFs in Figure \ref{fig:CDF1}, the LR statistic distributions are conservative are better approximated by a $\chi_0^2/2 + \chi_1^2/2$ distribution than the corresponding distributions without likelihood modification.  Tree topology made little difference as both the LR statistic and modified LR statistic CDFs were similar when data were simulated with different topologies.  Figure \ref{fig:CDF5taxaTL3} and supplementary Figures \ref{fig:CDF5taxaTL6} - \ref{fig:CDF32taxaTL6} contain the LR statistic CDFs for the remaining simulation scenarios.
\begin{figure}
    \centering
<<CDFNull1FBTL6,echo=F,warning=F>>=
    rm(list=ls())
    sim5.lnls <- read.csv("../../sims/sim5/lnLs.csv")
    sim6.lnls <- read.csv("../../sims/sim6/lnLs.csv")
    sim10.lnls <- read.csv("../../sims/sim10/lnLs.csv")
    sim11.lnls <- read.csv("../../sims/sim11/lnLs.csv")

    lrs1.sim5 <- sort(2*(sim5.lnls$bsA-sim5.lnls$null1))
    lrs2.sim5 <- sort(2*(sim5.lnls$bsA-sim5.lnls$null2))
    lrs1.sim6 <- sort(2*(sim6.lnls$bsA-sim6.lnls$null1))
    lrs2.sim6 <- sort(2*(sim6.lnls$bsA-sim6.lnls$null2))
    lrs1.sim10 <- sort(2*(sim10.lnls$bsA-sim10.lnls$null1))
    lrs2.sim10 <- sort(2*(sim10.lnls$bsA-sim10.lnls$null2))
    lrs1.sim11 <- sort(2*(sim11.lnls$bsA-sim11.lnls$null1))
    lrs2.sim11 <- sort(2*(sim11.lnls$bsA-sim11.lnls$null2))

    N <- length(sim5.lnls$bsA)
    x <- seq(0,6.3,length.out=N)

    lrs <- c(x,lrs1.sim5,lrs2.sim5,
             x,lrs1.sim6,lrs2.sim6,
             x,lrs1.sim10,lrs2.sim10,
             x,lrs1.sim11,lrs2.sim11)

    prob.mx <- 1/2+pchisq(x,1)/2
    cprob.mx <- rep(c(prob.mx,rep(1:N/N,2)),4)

    prob.x <- pchisq(x,1);
    cprob.x <- rep(c(prob.x,rep(1:N/N,2)),4)

    cdf.data <- data.frame(lrs,cprob.mx,
                           lfg=rep(factor(c('Short','Long'),levels=c('Short','Long')),each=3*N,times=2),
                           pfg=rep(factor(c('Terminal','Internal'),levels=c('Terminal','Internal')),each=3*N*2),
                           model=rep(c('MixChi','LR1','LR2'),each=N,times=4))

    cdf.plot <- ggplot(cdf.data,aes(lrs,cprob.mx)) +
        coord_cartesian(xlim=c(0,6), ylim=c(0.5,1)) +
        labs(x="LRS",y=expression("P(X"<="x)")) +
        ##geom_line(aes(color=model,linetype=model),size=.5) +
        geom_line(aes(linetype=model),size=.5) +
        ##scale_color_manual(values=c("black","black","black")) +
        scale_linetype_manual(values=c("dashed","dotted","solid","solid"),labels=c('LR1','LR2',expression(chi[0]^2/2 + chi[1]^2/2),expression(chi[1]))) +
...
@
  \caption[]{CDFs of LR statistics under branch-site Model A for four simulation settings.  For each setting, 5,000 sequence alignments were generated with equally weighted site classes, $\omega=0$ and $\omega=1$, using a balanced, 8-taxon tree topology with branch lengths summing to 6.  The single foreground branch is either a terminal (\emph{Terminal} row) or internal (\emph{Internal} row) branch that is either 1/10th (\emph{Short} column) or 10 times (\emph{Long} column) the length of the other branches.  CDFs for $\chi^2_0/2 + \chi^2_1/2$ are also included.}
  \label{fig:CDFNull1FBTL6}
\end{figure}

\begin{figure}
    \centering
<<CDFNull2FBTL6,echo=F,warning=F>>=
    rm(list=ls())
    sim5.lnls <- read.csv("../../sims/sim5/lnLs.csv")
    sim6.lnls <- read.csv("../../sims/sim6/lnLs.csv")
    sim10.lnls <- read.csv("../../sims/sim10/lnLs.csv")
    ...
    cprob <- rep(c(prob.t,rep(1:N/N,2)),4)

    cdf.data <- data.frame(lrs,cprob,
                           lfg=rep(factor(c('Short','Long'),levels=c('Short','Long')),each=3*N,times=2),
                           pfg=rep(factor(c('Terminal','Internal'),levels=c('Terminal','Internal')),each=3*N*2),
                           model=rep(c('Theory','LR1','LR2'),each=N,times=4))
    ...
    cdf.plot <- ggplot(cdf.data,aes(lrs,cprob)) +
        ...
        geom_line(aes(linetype=model),size=.5) +
        ##scale_color_manual(values=c("black","black","black")) +
        scale_linetype_manual(values=c("dashed","dotted","solid"),labels=c('LR1','LR2',expression(chi[0]^2/2 + chi[1]^2/2))) +
        scale_y_continuous(breaks=scales::pretty_breaks(n=3)) +
        facet_grid(pfg~lfg)
    ...
    cdf.plot +
        theme(panel.spacing=unit(0,"lines"),
              ...
              panel.border = element_rect(colour = "black", fill=NA, size=1))
@
  \caption[]{CDFs of LR statistics under branch-site model A for four simulation settings.  For each simulation setting, 5,000 sequence alignments were generated with two site classes, $\omega<1$ and $\omega=1$ using a balanced, 32-taxon tree topology with branch lengths summing to 9.  The value of $\omega_0$ and its weight, $p_0$, used to generate the data are shown as column and row labels.  CDFs for $\chi^2_0/2 + \chi^2_1/2$ are also included.}
  \label{fig:CDF2}
\end{figure}
...
\begin{figure}
    ...
    p0_0.25_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.25_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.25_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.25_c2_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_c2_lnLs.csv",sep=',')

    p0_0.5_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.25_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.25_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.25_c2_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.5_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.5_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.5_c2_lnLs.csv",sep=',')

    p0_0.75_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.25_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.25_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.25_c2_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.5_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.5_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.5_c2_lnLs.csv",sep=',')
...
%The slightly larger than expected false positive rates are likely, at least in part, due to implementation issues.  For example, for one simulation under the null used for calibration, 98 of the 10,000, LR test statistics were less than 1, indicating that optimization did not complete with maximum likelihood parameters under the alternative model.
\begin{sidewaystable}
...
\begin{figure}
  \centering
<<powerPlots,echo=FALSE,cache=F>>=
lnLs.5taxa.tl3 <- read.csv("../../sim/alt/5_taxa_balanced_tree/data/lnLs.csv")
lnLs.10taxa.tl3 <- read.csv("../../sim/alt/10_taxa_balanced_tree/data/lnLs.csv")
lnLs.32taxa.tl3 <- read.csv("../../sim/alt/32_taxa_balanced_tree/data/lnLs.csv")
...
%\subsubsection{Identifiability Issues not Corrected by Likelihood Modification}
...
\begin{figure}
    ...
    p0_0.25_w0_0.25_m2a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.25_m2a_mles.csv",header=F)
    colnames(p0_0.25_w0_0.25_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.25_w0_0.5_m2a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_m2a_mles.csv",header=F)
    colnames(p0_0.25_w0_0.5_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.5_w0_0.25_m2a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.25_m2a_mles.csv",header=F)
    colnames(p0_0.5_w0_0.25_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.5_w0_0.5_m2a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.5_m2a_mles.csv",header=F)
    colnames(p0_0.5_w0_0.5_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.75_w0_0.25_m2a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.25_m2a_mles.csv",header=F)
    colnames(p0_0.75_w0_0.25_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.75_w0_0.5_m2a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.5_m2a_mles.csv",header=F)
    colnames(p0_0.75_w0_0.5_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')

    p0_0.25_w0_0.25_c2_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.25_c2_mles.csv",header=F)
    colnames(p0_0.25_w0_0.25_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.25_w0_0.5_c2_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_c2_mles.csv",header=F)
    colnames(p0_0.25_w0_0.5_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.5_w0_0.25_c2_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.25_c2_mles.csv",header=F)
    colnames(p0_0.5_w0_0.25_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.5_w0_0.5_c2_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.5_c2_mles.csv",header=F)
    colnames(p0_0.5_w0_0.5_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.75_w0_0.25_c2_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.25_c2_mles.csv",header=F)
    colnames(p0_0.75_w0_0.25_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    p0_0.75_w0_0.5_c2_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.5_c2_mles.csv",header=F)
...
\begin{figure}
    ...
    ## large p0 estimates under M2a are not associated with w0 values close to 1
    ##drop2 <- which(p0_0.25_w0_0.5_m2a_mles$p0>0.9)
    ##p0_0.25_w0_0.5_m2a_filtered_mles <- p0_0.25_w0_0.5_m2a_mles[-drop2,]
    ##colnames(p0_0.25_w0_0.5_m2a_filtered_mles) <- c('k','p0','p1','p2','w0','w1','w2')
    ##p0_0.25_w0_0.5_5_taxa_m1a_filtered_lnl <- p0_0.25_w0_0.5_5_taxa_m1a_lnl[-drop2]
    ##p0_0.25_w0_0.5_5_taxa_m2a_filtered_lnl <- p0_0.25_w0_0.5_5_taxa_m2a_lnl[-drop2]
    ##lrs_p0_0.25_w0_0.5_5_taxa_m2a_filtered <- sort(2*(p0_0.25_w0_0.5_5_taxa_m2a_filtered_lnl-p0_0.25_w0_0.5_5_taxa_m1a_filtered_lnl))
...
\begin{figure}
  \centering
<<klp0.25w0.5, echo=F>>=
site.lik.truth <- read.csv("../../sim/null/5_taxa_bl_3_10000_codons/data/sim_p0_0.25_w0_0.5/site_lik_truth.csv",header=F)
...
##rnames <- cnames <- seq(0,1,.1)
##kl <- matrix(rep(0,121),nrow=11,ncol=11,dimnames=list(rnames,cnames))
##vkl <- matrix(rep(0,121),nrow=11,ncol=11,dimnames=list(rev(rnames),cnames))
kl <- numeric(0)
vkl <- numeric(0)
for (p in seq(0,1,.1)) {
    for (w in seq(0,1,.1)) {
        site.lik <- read.csv(paste("../../sim/null/5_taxa_bl_3_10000_codons/data/sim_p0_0.25_w0_0.5/site_like_p_",p,"_w_",w,".csv",sep=''),header=F)
        y <- site.lik[,2]
        y[y==0] <- 1e-12
...
##lb <- kl - 2*sqrt(vkl/1000)
##ub <- kl + 2*sqrt(vkl/1000)
p <- rep(seq(0,1,.1),each=11)
w <- rep(seq(0,1,.1),times=11)
##kl.data <- data.frame(kl,lb,ub,w,p)

##kl.plot <- ggplot(kl.data,aes(p,kl)) +
##    ##coord_cartesian(xlim=c(0,1), ylim=c(0,.01)) +
##    labs(x=expression("p"[0]),y="KL Divergence") +
##    geom_point() +
##    geom_hline(yintercept=0) +
##    facet_wrap(~w,ncol=3,scales="free_y",labeller=label_bquote(cols=omega[0]*'='*.(w))) +
##    ##facet_wrap(~w,ncol=3,labeller=label_bquote(cols=omega[0]*'='*.(w))) +
##    geom_errorbar(aes(ymin=lb,ymax=ub)) +
##    ggtitle(expression(paste(p[0],"=0.25, ",omega[0],"=0.5, n=1000")))
...
kl.plot <- ggplot(kl.data,aes(p,kl)) +
    ...
    geom_point() +
    ##geom_hline(yintercept=0) + facet_wrap(~w,ncol=3,scales="free_y",labeller=label_bquote(cols=omega[0]*'='*.(w))) +
    geom_hline(yintercept=0) + facet_wrap(~w,ncol=3,labeller=label_bquote(cols=omega[0]*'='*.(w))) +
    coord_cartesian(ylim = c(-0.001, 0.01)) +
    geom_errorbar(aes(ymin=lb,ymax=ub))
    #ggtitle(expression(paste(p[0],"=0.25, ",omega[0],"=0.5, n=10000")))
...
##ggplot(kl.data, aes(p,w,z=kl)) + geom_tile(aes(fill=kl)) + scale_fill_gradient(low="white", high="green")
@
...
% old
% Furthermore, the very small KL divergences were specific to certain parameter settings; generating with other mixing distributions gives larger, positive KL divergences when fixing mixing distributions distant from the generating distribution.  For example, in Figure \ref{fig:KLp0.75w0.5}, which shows the KL divergences for data generated with $(p_0,\omega_0)=(0.75,0.5)$, the only KL divergences around 0 are for parameter values near the generating values.
% old
% \cmt{I'm pretty sure we want to remove this paragraph:} For our final investigation of the identifiability issues, we extracted the 125 sites generated with $\omega=0.5$ from the difficult simulation scenario and calculated likelihoods scores for each of these 10,000 datasets.  Figure \ref{fig:kl.m0.w0.5} shows KL divergences between model M0 LR statistic distributions with $\omega$ fixed to the generating value of $0.5$ and the $\omega$ parameter fixed to varying values between $0$ and $1$.  The inset in Figure \ref{fig:kl.m0.w0.5} shows that even fixed $\omega$ values close to the generating value have LR statistic distributions different than when $\omega$ is fixed at $0.5$.  By eliminating the uncertainty involved with assigning sites to site classes....

% ********************* new ******************
%We repeated KL divergence calculations for all pairs of taxa in the 5-taxon tree for mixing distributions $(p_0,\omega_0)=(1,0.75)$ and $(p_0,\omega_0)=(0.5,0.5)$.  The maximum KL divergence was again small $(0.00085)$, suggesting that $p(x;\omega=0.75,\zeta) \approx p(x;\omega=0.5,\zeta)/2 + p(x;\omega=1,\zeta)/2$.  Multiplying this equation by $p_0$ and rearranging gives $p(x;\omega=0.75,\zeta)p_0 + p(x;\omega=1,\zeta)(1-p_0)  \approx p(x;\omega=0.5,\zeta)p_0/2 + p(x;\omega=1,\zeta)/(1-p_0/2)$.  That is, a range of mixing distributions give rise to pattern probabilities that are difficult to distinguish, which can also be seen in Figure  \ref{fig:KLp0.25w0.5}.  However, because $p_0/2$ can be at most $0.5$, pattern probabilities generated from, e.g., $(p_0,\omega_0)=(0.75,0.5)$ are not consistent with $(p_0,\omega_0)=(p0,\omega=0.75)$ as shown in Figure \ref{fig:KLp0.75w0.5}.

%While we have shown that there are regions of mixing-distribution parameter space that can make estimation and inference difficult, Figures \ref{fig:KLp0.25w0.5} and \ref{fig:KLp0.75w0.5} suggest that distributions with small $\omega_0$ do not give rise to site patterns similar to $(p_0,\omega_0)=(0.25,0.5)$.  Furthermore, the good behaviour of the modified LR statistic when used with trees with more taxa and longer branch lengths (e.g., Fig. \ref{fig:CDF32taxaTL9}) suggests these problems are likely restricted to trees with fewer taxa and shorter branch lengths.
% ********************* new ******************
...
% ********************* old ******************
%A surprising simulation result is the lack of symmetry of mixing distributions that give very similar site patterns.  For instance, one might expect the mixing distribution with $(p_0,\omega_0)=(0.75,0.5)$ to have similar difficulties to $(p_0,\omega_0)=(0.25,0.5)$, because the former distribution simply reverses the $\omega$ having mass $0.25$ from $\omega=0.5$ to $\omega=1$.  This turns out not to be the case.  An intuitive explanation for this phenomenon comes from the expectation that site patterns generated with some $\omega$ value may be consistent with site patterns generated from a range of other $\omega$ values.  With fewer taxa and smaller branch lengths, the range of $\omega$ values is expected to be larger.  If, for example, sites were generated with $\omega=0.75$, approximately half of those sites are expected to show fewer than the median number of nonsynonymous differences and thus might give sites patterns consistent with those that tend to be generated with $\omega=0.5$.  Similarly, the other half are expected to show more than the median number of nonsynonymous differences and might give site patterns similar to those generated with $\omega=1.0$.  This explanation is consistent with results we obtained by fitting $(p_0,\omega_0)=(1,0.75)$ and $(p_0,\omega_0)=(0.5,0.5)$ to all $61^2$ sites patterns and branch lengths from the $5$-taxon tree used in the other simulation studies.  The maximum KL divergence was $0.00085$, suggesting, more generally, that $(p_0,0.75)$ gives pattern probabilities consistent with $(p_0/2,0.5)$.  Since $p_0/2$ can be no larger than $0.5$, $(p_0,0.75)$ can not give rise to pattern probabilities consistent with $(p_0,\omega_0)=(0.75.0.5)$.
% ********************* old ******************
%This explanation is consistent with the KL divergence plots in Figure \ref{fig:KLp0.25w0.5} where $(p_0,\omega_0)=(0.75,0.5)$ resulted in KL divergences near 0.  However, since half of the sites generated with $\omega=0.75$ are expected to be similar to sites generated with $\omega=0.5$, the maximum $p_0$ on $\omega_0=0.5$ that can be expected to be explained by a distribution with $w_0=0.75$ is $p_0=0.5$.  As confirmed by Figure \ref{fig:KLp0.75w0.5}, no generating distribution is consistent with sequence alignments generated with $75\%$ of sites under $\omega=0.5$ and the remaining sites under $\omega=1$.

%site patterns could be consistent with site patterns generated with $\omega=0.5$ and half with sites patterns generated with $\omega=1$.  This could lead to sequence alignments indistinguishable from those for the difficult scenario with $25\%$ of the sites generated with $\omega=0.5$.  This theory is consistent with the KL divergence plots in Figure \ref{fig:KLp0.25w0.5} where $\approx 50\%$ of sites under $\omega \approx 0.75$ resulted in KL divergence near 0.  As confirmed by Figure \ref{fig:KLp0.75w0.5}, no such generating distribution is consistent with sequence alignments generated with $75\%$ of sites under $\omega=0.5$ and the remaining sites under $\omega=1$.
%Problems arose with $25\%$ but not $75\%$ of the sites generated with $\omega=0.5$ and the remaining sites with $\omega=1$.

%Furthermore, when sites are generated with $\omega=0.5$, there is more flexibility for these site patterns to be consistent with larger rather than smaller values of $\omega$ (Fig. \ref{fig:kl.m0.w0.5}).  When few sites (e.g., 25\% of 500) are generated with $\omega=0.5$, enough of these sites may skew a site class around $\omega \approx 1$ rather than constitute a second site class around $\omega \approx 0.5$.  On the other hand, when more sites (e.g., 75\% of 500) are generated under $\omega=0.5$, even when some were consistent with a larger $\omega$, enough would remain to require weight on a class of sites around an $\omega$ near $0.5$.  \cmt{This next sentence needs elaboration: } Under M2a, having few sites generated with $\omega=1$ when the remaining sites are generated with an $\omega$ sufficiently smaller than $1$ is not problematic, because, unlike the $\omega<1$ site class, the $\omega=1$ value is fixed.

% For each of the six simulations scenarios with varying weights and values for two site classes, $\omega<1$ and $\omega=1$, 10,000, 500-codon sequence alignments were generated using balanced, 5-, 10-, and 32-taxon tree topologies with branch lengths summing to 3, 6, or 9.  The value of $\omega_0$ and its weight, $p_0$, used to generate the data are shown in column and row labels.  Modified likelihood tuning parameters of $C=0$ (no likelihood modification) and $C=2$ were used.  The LR statistics were compared to $\chi_0^2/2 + \chi_1^2/2$ distributions using a p-value cut-off of $\alpha=0.05$
...
%     \caption{False positive rates.}
%     \centering
%     \begin{tabular}[h!]{*{2}l*{6}c}
%       \toprule
%       Tuning Parameter   &  & \multicolumn{3}{c}{$\omega_0=.25$} & \multicolumn{3}{c}{$\omega_0=.5$} \\
%       \cmidrule(lr){1-1}  \cmidrule(lr){3-5}                   \cmidrule(lr){6-8}
%                          & $p_0=$ & $.25$ & $.5$ & $.75$               & $.25$ & $.5$ & $.75$              \\
%       \cmidrule(lr){3-3} \cmidrule(lr){4-4} \cmidrule(lr){5-5} \cmidrule(lr){6-6} \cmidrule(lr){7-7} \cmidrule(lr){8-8}
%       $C=0$              &  & .10   & .09  & .09                 & .10   & .08  & .07                \\
%       $C=2$              &  & .06   & .06  & .05                 & .08   & .06  & .06                \\
%       \bottomrule
%     \end{tabular}
%     \label{tab:FPRates}
%     \begin{tablenotes}
%       \small
%     \item  False positive rates for LR tests of positive selection under nested models M1a/M2a with and without likelihood modification.  For each of six simulations, 10,000 sequence alignments were generated with two site classes, $\omega<1$ and $\omega=1$ using a balanced, 5-taxon tree topology with branch lengths summing to 3.  The value of $\omega_0$ and its weight, $p_0$, used to generate the data are shown as column and row labels.  Modified likelihood tuning parameters of $C=0$ (no likelihood modification) and $C=2$ were used.
%     \end{tablenotes}
%   \end{threeparttable}
% \end{table}
...
%  \centering
%  \begin{threeparttable}
%    \caption{Power with/without likelihood modification (calibrated).}
% \begin{table}
%   \caption{\cmt{To be removed or moved to supplementary materials} \small Power of likelihood ratio (LR) tests under model pair M2a/M1a without (C=0) and with (C=2) likelihood modification.  For each simulation setting, 10,000 datasets were simulated using the indicated number of taxa, tree length, and an $\omega$ distribution with 0.45 weight on $\omega=0$, $p_2$ on $\omega_2$, and the remaining weight on $\omega=1$.  Reported are the proportion of LR tests out of 10,000 for which positive selection was indicated at a significance level of 0.05.  The significance thresholds without and with likelihood modification were estimated by simulating under the null distribution (0.45 weight on $\omega=0$ and 0.55 weight on $\omega=1$) and using the 95th percentile of the LR statistic distributions.}
%   \resizebox{\textwidth}{!}{%
%     \begin{tabular}[h!]{ll*{5}c*{5}c*{5}c}
%       \toprule
%       & & \multicolumn{15}{c}{{\bf \textit{Tree Length 3}}} \\
%       \cmidrule(lr){3-17}
%       & & \multicolumn{5}{c}{{\bf \textit{5 taxa}}} & \multicolumn{5}{c}{{\bf \textit{10 taxa}}} & \multicolumn{5}{c}{{\bf \textit{32 taxa}}} \\
%       \cmidrule(lr){3-7} \cmidrule(lr){8-12} \cmidrule(lr){13-17}
%       & & \multicolumn{5}{c}{{\bf \textit{$p_2$}}}  & \multicolumn{5}{c}{{\bf \textit{$p_2$}}}   & \multicolumn{5}{c}{{\bf \textit{$p_2$}}} \\
%       \cmidrule(lr){3-7} \cmidrule(lr){8-12}  \cmidrule(lr){13-17}
%       Penalty & $\omega_2$ & 0.02 & 0.04 & 0.06 & 0.08 & 0.1 & 0.02 & 0.04 & 0.06 & 0.08 & 0.1 & 0.02 & 0.04 & 0.06 & 0.08 & 0.1 \\
...
%       C=0 & 1.05 & 0.050 & 0.050 & 0.051 & 0.051 & 0.050 & 0.050 & 0.050 & 0.051 & 0.051 & 0.050 & 0.050 & 0.050 & 0.051 & 0.051 & 0.050\\
%           & 1.2  & 0.052 & 0.055 & 0.061 & 0.063 & 0.066 & 0.052 & 0.055 & 0.061 & 0.063 & 0.066 & 0.052 & 0.055 & 0.061 & 0.063 & 0.066\\
%           & 1.5  & 0.066 & 0.087 & 0.113 & 0.140 & 0.173 & 0.066 & 0.087 & 0.113 & 0.140 & 0.173 & 0.066 & 0.087 & 0.113 & 0.140 & 0.173\\
%           & 5    & 0.739 & 0.977 & 1.000 & 1.000 & 1.000 & 0.739 & 0.977 & 1.000 & 1.000 & 1.000 & 0.739 & 0.977 & 1.000 & 1.000 & 1.000\\
%       C=2 & 1.05 & 0.051 & 0.052 & 0.053 & 0.053 & 0.053 & 0.051 & 0.052 & 0.053 & 0.053 & 0.053 & 0.051 & 0.052 & 0.053 & 0.053 & 0.053\\
%           & 1.2  & 0.054 & 0.057 & 0.062 & 0.065 & 0.072 & 0.054 & 0.057 & 0.062 & 0.065 & 0.072 & 0.054 & 0.057 & 0.062 & 0.065 & 0.072\\
%           & 1.5  & 0.066 & 0.090 & 0.113 & 0.143 & 0.178 & 0.066 & 0.090 & 0.113 & 0.143 & 0.178 & 0.066 & 0.090 & 0.113 & 0.143 & 0.178\\
%           & 5    & 0.669 & 0.962 & 0.999 & 1.000 & 1.000 & 0.669 & 0.962 & 0.999 & 1.000 & 1.000 & 0.669 & 0.962 & 0.999 & 1.000 & 1.000\\
%       & & \multicolumn{15}{c}{{\bf \textit{Tree Length 6}}} \\
%       \cmidrule(lr){3-17}
%       & & \multicolumn{5}{c}{{\bf \textit{5 taxa}}} & \multicolumn{5}{c}{{\bf \textit{10 taxa}}} & \multicolumn{5}{c}{{\bf \textit{32 taxa}}} \\
%       \cmidrule(lr){3-7} \cmidrule(lr){8-12} \cmidrule(lr){13-17}
%       & & \multicolumn{5}{c}{{\bf \textit{$p_2$}}}  & \multicolumn{5}{c}{{\bf \textit{$p_2$}}}   & \multicolumn{5}{c}{{\bf \textit{$p_2$}}} \\
...
%           & 1.5  & 0.066 & 0.090 & 0.113 & 0.143 & 0.178 & 0.066 & 0.090 & 0.113 & 0.143 & 0.178 & 0.066 & 0.090 & 0.113 & 0.143 & 0.178\\
%           & 5    & 0.669 & 0.962 & 0.999 & 1.000 & 1.000 & 0.669 & 0.962 & 0.999 & 1.000 & 1.000 & 0.669 & 0.962 & 0.999 & 1.000 & 1.000\\
%       & & \multicolumn{15}{c}{{\bf \textit{Tree Length 9}}} \\
%       \cmidrule(lr){3-17}
%       & & \multicolumn{5}{c}{{\bf \textit{5 taxa}}} & \multicolumn{5}{c}{{\bf \textit{10 taxa}}} & \multicolumn{5}{c}{{\bf \textit{32 taxa}}} \\
%       \cmidrule(lr){3-7} \cmidrule(lr){8-12} \cmidrule(lr){13-17}
%       & & \multicolumn{5}{c}{{\bf \textit{$p_2$}}}  & \multicolumn{5}{c}{{\bf \textit{$p_2$}}}   & \multicolumn{5}{c}{{\bf \textit{$p_2$}}} \\
...
%     \caption{Spearman rank correlations of site posterior probabilities for different methods of classification under model M2a.}
%     \centering
%     \begin{tabular}[!ht]{*{6}l}
%       \toprule
%       Gene                       & N*/N & N*/B & N*/S & N/S  & B/S \\
%       \cmidrule(lr){1-1} \cmidrule(lr){2-2} \cmidrule(lr){3-3} \cmidrule(lr){4-4} \cmidrule(lr){5-5} \cmidrule(lr){6-6}
%       \textit{CDH3}              & 0.40 & 1.00 & 1.00 & 0.40 & 1.00 \\
%       \textit{mivN}              & 0.76 & 0.99 & 0.97 & 0.78 & 0.96 \\
%       \textit{pgpA}              & 0.71 & 0.99 & 0.99 & 0.73 & 0.98 \\
%       \textit{TrbL-VirB6\_2}     & 0.72 & 1.00 & 0.98 & 0.72 & 0.98 \\
%       \hline
%       lysin                      & 1.00 & 1.00 & 0.99 & 0.99 & 0.99 \\
%       \textit{nuoL3}             & 1.00 & 0.99 & 0.90 & 0.90 & 0.93 \\
%       \textit{pol}               & 0.94 & 0.96 & 0.79 & 0.76 & 0.85 \\
%       \textit{RfaL}              & 1.00 & 1.00 & 0.97 & 0.96 & 0.97 \\
%       \textit{TrbL-VirB6\_3}     & 0.98 & 0.98 & 0.91 & 0.93 & 0.93 \\
%       \textit{vif}               & 1.00 & 1.00 & 0.97 & 0.97 & 0.98 \\
%       \hline
%       $\beta$-globin             & 0.96 & 0.94 & 0.85 & 0.82 & 0.90 \\
%       \textit{ccmF}              & 0.93 & 0.93 & 0.87 & 0.80 & 0.96 \\
%       \textit{ENAM}              & 1.00 & 1.00 & 1.00 & 1.00 & 1.00 \\
%       \textit{env}               & 0.99 & 1.00 & 0.99 & 0.98 & 0.99 \\
%       \textit{perM}              & 0.99 & 1.00 & 0.92 & 0.91 & 0.93 \\
%       \bottomrule
%     \end{tabular}
%     \label{tab:sitecors}
%     \begin{tablenotes}
...
% \begin{sidewaystable}
%   \begin{threeparttable}
%     \caption{Sites identified to be under positive selection only with likelihood modification for 15 genes.}
%     \centering
%     \begin{tabular}[]{*{13}l}
%       \toprule
%       \multicolumn{1}{c}{Gene} & \multicolumn{4}{c}{NEB/NEB(C=2)} & \multicolumn{4}{c}{BEB/BEB(C=2)} & \multicolumn{4}{c}{SBA/NEB(C=2)} \\
%       \cmidrule(lr){1-1} \cmidrule(lr){2-5} \cmidrule(lr){6-9} \cmidrule(lr){10-13}
%                              & ++ & +- & -+ & -- & ++ & +- & -+ & -- & ++ & +- & -+ & -- \\
%       $\beta$-globin         & 4  & 0  & 1  &    & 5  & 0  & 0  &    & 5  &    &    &    \\
%       \textit{ccmF}          & 7  & 0  & 4  &    & 11 & 0  & 0  &    & 0  &    &    &    \\
%       \textit{CDH3}          & 1  & 0  & 30 &    & 12 & 0  & 0  &    & 1  &    &    &    \\
%       \textit{ENAM}          & 9  & 0  & 37 &    & 21 & 0  & 0  &    & 0  &    &    &    \\
%       \textit{env}           & 14 & 0  & 13 &    & 16 & 0  & 1  &    & 4  &    &    &    \\
%       lysin                  & 33 & 0  & 2  &    & 32 & 0  & 0  &    & 1  &    &    &    \\
%       \textit{mivN}          & 0  & 1  & 0  &    & 7  & 0  & 0  &    & 1  &    &    &    \\
%       \textit{nuoL3}         & 18 & 0  & 0  &    & 18 & 0  & 0  &    & 3  &    &    &    \\
%       \textit{perM}          & 4  & 0  & 2  &    & 6  & 0  & 0  &    & 0  &    &    &    \\
%       \textit{pgpA}          & 0  & 1  & 0  &    & 4  & 0  & 0  &    & 1  &    &    &    \\
%       \textit{pol}           & 12 & 0  & 2  &    & 19 & 0  & 0  &    & 7  &    &    &    \\
%       \textit{RfaL}          & 20 & 0  & 6  &    & 20 & 0  & 0  &    & 13 &    &    &    \\
%       \textit{TrbL-VirB6\_2} & 0  & 0  & 0  &    & 16 & 0  & 0  &    & 0  &    &    &    \\
%       \textit{TrbL-VirB6\_3} & 28 & 0  & 3  &    & 27 & 0  & 0  &    & 1  &    &    &    \\
%       \textit{vif}           & 13 & 0  & 1  &    & 13 & 0  & 0  &    & 2  &    &    &    \\
%       \bottomrule
%     \end{tabular}
%     \label{tab:SitesIded}
%     \begin{tablenotes}
...
% \begin{table}[H]
%   \begin{threeparttable}
%     \caption{Spearman rank correlations between site posterior probabilities for different methods of classification under model M2a.}
%     \centering
...
%   \centering
% <<klp0.5w0.5, echo=F>>=
% site.lik.truth <- read.csv("../../sim/null/5_taxa_bl_3_10000_codons/data/sim_p0_0.5_w0_0.5/site_lik_truth.csv",header=F)
% colnames(site.lik.truth) <- c('cnt','lik')
% n <- sum(site.lik.truth$cnt)
% cnt <- site.lik.truth$cnt
% y.truth <- site.lik.truth$lik
...
% for (p in seq(0,1,.1)) {
%     for (w in seq(0,1,.1)) {
%         site.lik <- read.csv(paste("../../sim/null/5_taxa_bl_3_10000_codons/data/sim_p0_0.5_w0_0.5/site_like_p_",p,"_w_",w,".csv",sep=''),header=F)
%         y <- site.lik[,2]
%         y[y==0] <- 1e-12
%         y.bar <- sum(cnt*log(y.truth / y))/n
%         kl <- c(kl,y.bar)
...
% ##ggplot(kl.data, aes(p,w,z=kl)) + geom_tile(aes(fill=kl)) + scale_fill_gradient(low="white", high="green")
% @
% \caption[]{Kullback-Leibler divergences between the distributions of site likelihoods with parameters fixed to the generating model and parameters fixed to varying weights on values of $\omega$ ranging from $0$ to $1$.  The date were generated under model M1a with $p_0=0.5$ of the sites simulated to evolve under $\omega_0=0.5$ and the remaining sites under $\omega=1$.  Error bars for two standard deviations above and below each Kullback-Leibler estimate are included. Points missing from each plot are above the visible range.}
% \label{fig:KLp0.5w0.5}
% \end{figure}
...
%   \centering
%   <<ml.m0.w0.5, echo=F>>=
%   t5.tl9 <- as.vector(apply(read.csv("../../sim/null//test_m0/data/m0_w0_sim_0.5_5_taxa_tl_9_all_w.csv",header=F),2,mean)[2:21])
%   t5.tl9.sd <- as.vector(apply(read.csv("../../sim/null//test_m0/data/m0_w0_sim_0.5_5_taxa_tl_9_all_w.csv",header=F),2,sd)[2:21])

%   x <- seq(0.05,1,0.05)
%   lb <- t5.tl9-qt(.975,999)*t5.tl9.sd/sqrt(1000)
%   ub <- t5.tl9+qt(.975,999)*t5.tl9.sd/sqrt(1000)

%   line.points <- data.frame(x1=c(0.4,0.7),x2=c(0.4,0.7),y1=c(-2359.619,-2361.350),y2=c(-2455,-2455))

%   ml.plot <- ggplot(data.frame(x,t5.tl9),aes(x,t5.tl9)) +
%       geom_point(shape=16,size=2) +
%       labs(x=expression(omega),y="mean lnL") +
%       geom_errorbar(aes(ymin=lb,ymax=ub)) +
%       geom_segment(data=line.points,aes(x=x1,xend=x2,y=y1,yend=y2))

%   ml.plot + theme(panel.spacing=unit(0,"lines"),
%                   panel.background=element_blank(),
%                   strip.background=element_blank(),
%                   legend.title=element_blank(),
%                   legend.key=element_rect(fill="transparent"),
...
%   z.df <- data.frame(x[8:15],t5.tl9[8:15])
%   z.lb <- lb[8:15]
%   z.ub <- ub[8:15]
%   colnames(z.df) <- c('w','lnL')
%   ml.plot.zoom <- ggplot(z.df,aes(w,lnL)) +
%       geom_point(shape=16,size=2) +
%       labs(x=expression(omega),y="mean lnL") +
%       geom_errorbar(aes(ymin=z.lb,ymax=z.ub))

%   vp <- viewport(width=0.365,height=0.365,x=0.522,y=0.65)

%   print(ml.plot.zoom + theme(panel.spacing=unit(0,"lines"),
%                              panel.background=element_blank(),
%                              strip.background=element_blank(),
%                              legend.title=element_blank(),
%                              legend.key=element_rect(fill="transparent"),
%                              text=element_text(size=12),
%                              panel.border = element_rect(colour = "black", fill=NA, size=1)), vp=vp)
% @
% \caption[]{Blah}
% \label{fig:ml.m0.w0.5}
% \end{figure}
...
%   \centering
%   <<kl.m0.w0.5, echo=F>>=
%   t5.tl9 <- read.csv("../../sim/null/test_m0/data/m0_w0_sim_0.5_5_taxa_tl_9_all_w.csv",header=F)[,2:21]

%   colnames(t5.tl9) <- seq(0.05,1,0.05)
%   t5.tl9.sd <- apply(t5.tl9[,]$'0.5'-t5.tl9,2,sd)
%   t5.tl9 <- apply(t5.tl9[,]$'0.5'-t5.tl9,2,mean)

%   lb <- t5.tl9-qt(.975,999)*t5.tl9.sd/sqrt(1000)
%   ub <- t5.tl9+qt(.975,999)*t5.tl9.sd/sqrt(1000)

%   w <- seq(0.05,1,0.05)
%   kl.data <- data.frame(kl=t5.tl9,w=w)

%   kl.plot <- ggplot(kl.data,aes(w,kl)) +
%       geom_point() +
%       geom_errorbar(aes(ymin=lb,ymax=ub)) +
%       labs(x=expression(omega),y='Kullback-Leibler Divergence')
...
%   z.df <- data.frame(w=w[8:15],kl=t5.tl9[8:15])
%   z.lb <- lb[8:15]
%   z.ub <- ub[8:15]
%   kl.plot.zoom <- ggplot(z.df,aes(w,kl)) +
%       geom_point(shape=16,size=2) +
%       labs(x=expression(omega),y="Kullback-Leibler Divergence") +
%       geom_errorbar(aes(ymin=z.lb,ymax=z.ub))

%   #vp <- viewport(width=0.5,height=0.5,x=0.737,y=0.42)

%   #print(kl.plot.zoom + theme(panel.spacing=unit(0,"lines"),
%   #                           panel.background=element_blank(),
%   #                           strip.background=element_blank(),
%   #                           legend.title=element_blank(),
%   #                           legend.key=element_rect(fill="transparent"),
...
%   #                           panel.border = element_rect(colour = "black", fill=NA, size=1)), vp=vp)
% @
% \caption[]{Kullback-Leibler divergences between the distributions of site likelihoods with parameters fixed to the generating model ($\omega=0.5$) and parameters fixed to varying values of $\omega$ ranging from $0$ to $1$.}
% \label{fig:kl.m0.w0.5}
% \end{figure}
...
%       Model                & $p_0=$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ \\
%       \cmidrule(lr){1-1} \cmidrule(lr){3-3} \cmidrule(lr){4-4} \cmidrule(lr){5-5} \cmidrule(lr){6-6} \cmidrule(lr){7-7} \cmidrule(lr){8-8} \cmidrule(lr){9-9} \cmidrule(lr){10-10} \cmidrule(lr){11-11} \cmidrule(lr){12-12} \cmidrule(lr){13-13} \cmidrule(lr){14-14} \cmidrule(lr){15-15} \cmidrule(lr){16-16} \cmidrule(lr){17-17} \cmidrule(lr){18-18} \cmidrule(lr){19-19} \cmidrule(lr){20-20}
%       M2a (C=0)          &        & .17   & .16  & .16   & .16   & .15  & .13   & .16   & .15  & .14   & .16   & .14  & .12   & .16   & .15  & .15   & .14   & .14  & .12 \\
%       M2a (C=2)          &        & .12   & .11  & .10   & .14   & .12  & .10   & .12   & .10  & .10   & .14   & .11  & .10   & .12   & .11  & .11   & .12   & .12  & .10 \\
%       \rule{0pt}{4ex}
%       &  & \multicolumn{18}{c}{Tree Length 6} \\
%       \cmidrule(lr){3-20}
%       &  & \multicolumn{6}{c}{5 taxa} & \multicolumn{6}{c}{10 taxa} & \multicolumn{6}{c}{32 taxa} \\
...
%                          & $p_0=$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ \\
%       \cmidrule(lr){3-3} \cmidrule(lr){4-4} \cmidrule(lr){5-5} \cmidrule(lr){6-6} \cmidrule(lr){7-7} \cmidrule(lr){8-8} \cmidrule(lr){9-9} \cmidrule(lr){10-10} \cmidrule(lr){11-11} \cmidrule(lr){12-12} \cmidrule(lr){13-13} \cmidrule(lr){14-14} \cmidrule(lr){15-15} \cmidrule(lr){16-16} \cmidrule(lr){17-17} \cmidrule(lr){18-18} \cmidrule(lr){19-19} \cmidrule(lr){20-20}
%       M2a (C=0)          &        & .18   & .17  & .16   & .18   & .16  & .15   & .16   & .15  & .15   & .17   & .15  & .14   & .17   & .16  & .15   & .16   & .16  & .14 \\
%       M2a (C=2)          &        & .11   & .11  & .11   & .14   & .12  & .12   & .10   & .10  & .10   & .13   & .11  & .10   & .11   & .11  & .10   & .13   & .12  & .11 \\
%       \rule{0pt}{4ex}
%       &  & \multicolumn{18}{c}{Tree Length 9} \\
%       \cmidrule(lr){3-20}
%       &  & \multicolumn{6}{c}{5 taxa} & \multicolumn{6}{c}{10 taxa} & \multicolumn{6}{c}{32 taxa} \\
...
%                          & $p_0=$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ & $.25$ & $.5$ & $.75$ \\
%       \cmidrule(lr){3-3} \cmidrule(lr){4-4} \cmidrule(lr){5-5} \cmidrule(lr){6-6} \cmidrule(lr){7-7} \cmidrule(lr){8-8} \cmidrule(lr){9-9} \cmidrule(lr){10-10} \cmidrule(lr){11-11} \cmidrule(lr){12-12} \cmidrule(lr){13-13} \cmidrule(lr){14-14} \cmidrule(lr){15-15} \cmidrule(lr){16-16} \cmidrule(lr){17-17} \cmidrule(lr){18-18} \cmidrule(lr){19-19} \cmidrule(lr){20-20}
%       M2a (C=0)          &        & .19   & .16  & .15   & .19   & .17  & .14   & .16   & .15  & .14   & .17   & .14  & .12   & .16   & .16  & .15   & .16   & .15  & .15 \\
%       M2a (C=2)          &        & .12   & .11  & .11   & .15   & .13  & .12   & .12   & .10  & .10   & .12   & .11  & .10   & .11   & .11  & .10   & .12   & .11  & .11 \\
%       \bottomrule
%     \end{tabular}
%     \label{tab:FPRates3}
%     \begin{tablenotes}
%       \small
%     \item  False positive rates for LR tests of positive selection under nested models M1a/M2a with and without likelihood modification.  For each of six simulations scenaries with varying weights and values for two site classes, $\omega<1$ and $\omega=1$, 10,000, 500-codon sequence alignments were generated using 5-, 10-, and 32-taxon tree topologies with branch lengths summing to 3, 6, or 9.  The value of $\omega_0$ and its weight, $p_0$, used to generate the data are shown in column and row labels.  Modified likelihood tuning parameters of $C=0$ (no likelihood modification) and $C=2$ were used.  The LR statistics were compared to $\chi_0^2/2 + \chi_1^2/2$ distributions using $\alpha=0.1$ significance thresholds.
%     \end{tablenotes}
%   \end{threeparttable}
% \end{sidewaystable}
...
% \begin{table}[H]
%   \begin{threeparttable}
%     \caption{Standard deviations/IQRs of the $p_0$ and $\omega_0$ estimates with and without likelihood modification for data simulated with different numbers of taxa and tree lengths.}
%     \centering
%     \begin{tabular}[h!]{l*{7}c}
%       \toprule
%       & \multicolumn{3}{c}{{\bf \textit{$p_0$}}} & \multicolumn{3}{c}{{\bf \textit{$\omega_0$}}} \\
%       \cmidrule(lr){2-4} \cmidrule(lr){5-7}
%       Tree length =                                &  3      & 6        & 9       & 3       & 6       & 9       \\
%       \cmidrule(lr){2-2} \cmidrule(lr){3-3} \cmidrule(lr){4-4} \cmidrule(lr){5-5} \cmidrule(lr){6-6} \cmidrule(lr){7-7}
%       M2a                                          &         &          &         &         &         &         \\
%       \multicolumn{1}{r}{(\# taxa $\downarrow$) 5} & .32/.58 & .27/.40  & .22/.28 & .24/.34 & .17/.23 & .13/.17 \\
%       \multicolumn{1}{r}{8}                        & .30/.50 & .19/.23  & .14/.17 & .23/.31 & .13/.17 & .03/.13 \\
%       \multicolumn{1}{r}{10}                       & .29/.47 & .18/.22  & .12/.15 & .23/.31 & .13/.16 & .09/.12 \\
%       \multicolumn{1}{r}{32}                       & .28/.40 & .xx/.xx  & .08/.11 & .22/.30 & .xx/.xx & .07/.09 \\
%       C2                                           &         &          &         &         &         &         \\
%       \multicolumn{1}{r}{(\# taxa $\downarrow$) 5} & .33/.64 & .24/.33  & .24/.34 & .24/.34 & .16/.22 & .14/.19 \\
%       \multicolumn{1}{r}{8}                        & .31/.55 & .22/.27  & .17/.20 & .24/.32 & .14/.18 & .11/.13 \\
%       \multicolumn{1}{r}{10}                       & .31/.55 & .21/.26  & .14/.17 & .28/.32 & .14/.18 & .09/.12 \\
%       \multicolumn{1}{r}{32}                       & .29/.49 & .xx/.xx  & .09/.11 & .22/.31 & .xx/.xx & .07/.09 \\
%       \bottomrule
%     \end{tabular}
%     \label{tab:p0_w0_spread}
%     \begin{tablenotes}
%       \small
%     \item
%     \end{tablenotes}
...
\begin{figure}[H]
    ...
    rm(list=ls())
    p0_0.25_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.25_w0_0.25_5_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.25_w0_0.25_5_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.25_w0_0.25_5_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.25_w0_0.5_5_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.25_w0_0.5_5_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.25_w0_0.5_5_taxa_tl_6_c2_lnLs.csv",sep=',')

    p0_0.5_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.5_w0_0.25_5_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.5_w0_0.25_5_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.5_w0_0.25_5_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.5_w0_0.5_5_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.5_w0_0.5_5_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.5_w0_0.5_5_taxa_tl_6_c2_lnLs.csv",sep=',')

    p0_0.75_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.75_w0_0.25_5_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.75_w0_0.25_5_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.75_w0_0.25_5_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.75_w0_0.5_5_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.75_w0_0.5_5_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_6/data/p0_0.75_w0_0.5_5_taxa_tl_6_c2_lnLs.csv",sep=',')
...
\begin{figure}[H]
    ...
    rm(list=ls())
    p0_0.25_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.25_w0_0.25_5_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.25_w0_0.25_5_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.25_w0_0.25_5_taxa_tl_9_c2_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.25_w0_0.5_5_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.25_w0_0.5_5_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.25_w0_0.5_5_taxa_tl_9_c2_lnLs.csv",sep=',')

    p0_0.5_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.5_w0_0.25_5_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.5_w0_0.25_5_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.5_w0_0.25_5_taxa_tl_9_c2_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.5_w0_0.5_5_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.5_w0_0.5_5_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.5_w0_0.5_5_taxa_tl_9_c2_lnLs.csv",sep=',')

    p0_0.75_w0_0.25_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.75_w0_0.25_5_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.75_w0_0.25_5_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.75_w0_0.25_5_taxa_tl_9_c2_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.75_w0_0.5_5_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.75_w0_0.5_5_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_9/data/p0_0.75_w0_0.5_5_taxa_tl_9_c2_lnLs.csv",sep=',')
...
\begin{figure}[H]
    ...
    rm(list=ls())
    p0_0.25_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.25_w0_0.25_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.25_w0_0.25_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.25_w0_0.25_c2_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.25_w0_0.5_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.25_w0_0.5_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.25_w0_0.5_c2_lnLs.csv",sep=',')

    p0_0.5_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.5_w0_0.25_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.5_w0_0.25_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.5_w0_0.25_c2_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.5_w0_0.5_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.5_w0_0.5_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.5_w0_0.5_c2_lnLs.csv",sep=',')

    p0_0.75_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.75_w0_0.25_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.75_w0_0.25_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.75_w0_0.25_c2_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.75_w0_0.5_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.75_w0_0.5_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_3/data/p0_0.75_w0_0.5_c2_lnLs.csv",sep=',')
...
\begin{figure}[H]
    ...
    rm(list=ls())
    p0_0.25_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.25_w0_0.25_10_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.25_w0_0.25_10_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.25_w0_0.25_10_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.25_w0_0.5_10_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.25_w0_0.5_10_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.25_w0_0.5_10_taxa_tl_6_c2_lnLs.csv",sep=',')

    p0_0.5_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.5_w0_0.25_10_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.5_w0_0.25_10_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.5_w0_0.25_10_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.5_w0_0.5_10_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.5_w0_0.5_10_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.5_w0_0.5_10_taxa_tl_6_c2_lnLs.csv",sep=',')

    p0_0.75_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.75_w0_0.25_10_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.75_w0_0.25_10_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.75_w0_0.25_10_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.75_w0_0.5_10_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.75_w0_0.5_10_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_6/data/p0_0.75_w0_0.5_10_taxa_tl_6_c2_lnLs.csv",sep=',')
...
\begin{figure}[H]
    ...
    rm(list=ls())
    p0_0.25_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.25_w0_0.25_10_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.25_w0_0.25_10_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.25_w0_0.25_10_taxa_tl_9_c2_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.25_w0_0.5_10_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.25_w0_0.5_10_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.25_w0_0.5_10_taxa_tl_9_c2_lnLs.csv",sep=',')

    p0_0.5_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.5_w0_0.25_10_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.5_w0_0.25_10_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.5_w0_0.25_10_taxa_tl_9_c2_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.5_w0_0.5_10_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.5_w0_0.5_10_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.5_w0_0.5_10_taxa_tl_9_c2_lnLs.csv",sep=',')

    p0_0.75_w0_0.25_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.75_w0_0.25_10_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.75_w0_0.25_10_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.75_w0_0.25_10_taxa_tl_9_c2_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_m1a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.75_w0_0.5_10_taxa_tl_9_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_m2a_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.75_w0_0.5_10_taxa_tl_9_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_10_taxa_c2_lnl <- scan("../../sim/null/10_taxa_bl_9/data/p0_0.75_w0_0.5_10_taxa_tl_9_c2_lnLs.csv",sep=',')
...
\begin{figure}[H]
    ...
    p0_0.25_w0_0.25_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.25_w0_0.25_32_taxa_tl_3_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.25_w0_0.25_32_taxa_tl_3_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.25_w0_0.25_32_taxa_tl_3_c2_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.25_w0_0.5_32_taxa_tl_3_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.25_w0_0.5_32_taxa_tl_3_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.25_w0_0.5_32_taxa_tl_3_c2_lnLs.csv",sep=',')

    p0_0.5_w0_0.25_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.5_w0_0.25_32_taxa_tl_3_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.5_w0_0.25_32_taxa_tl_3_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.5_w0_0.25_32_taxa_tl_3_c2_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.5_w0_0.5_32_taxa_tl_3_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.5_w0_0.5_32_taxa_tl_3_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.5_w0_0.5_32_taxa_tl_3_c2_lnLs.csv",sep=',')

    p0_0.75_w0_0.25_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.75_w0_0.25_32_taxa_tl_3_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.75_w0_0.25_32_taxa_tl_3_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.75_w0_0.25_32_taxa_tl_3_c2_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.75_w0_0.5_32_taxa_tl_3_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.75_w0_0.5_32_taxa_tl_3_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_3/data/p0_0.75_w0_0.5_32_taxa_tl_3_c2_lnLs.csv",sep=',')
...
\begin{figure}[H]
    ...
    rm(list=ls())
    p0_0.25_w0_0.25_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.25_w0_0.25_32_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.25_w0_0.25_32_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.25_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.25_w0_0.25_32_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.25_w0_0.5_32_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.25_w0_0.5_32_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.25_w0_0.5_32_taxa_tl_6_c2_lnLs.csv",sep=',')

    p0_0.5_w0_0.25_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.5_w0_0.25_32_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.5_w0_0.25_32_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.25_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.5_w0_0.25_32_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.5_w0_0.5_32_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.5_w0_0.5_32_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.5_w0_0.5_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.5_w0_0.5_32_taxa_tl_6_c2_lnLs.csv",sep=',')

    p0_0.75_w0_0.25_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.75_w0_0.25_32_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.75_w0_0.25_32_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.25_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.75_w0_0.25_32_taxa_tl_6_c2_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.75_w0_0.5_32_taxa_tl_6_m1a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.75_w0_0.5_32_taxa_tl_6_m2a_lnLs.csv",sep=',')
    p0_0.75_w0_0.5_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_bl_6/data/p0_0.75_w0_0.5_32_taxa_tl_6_c2_lnLs.csv",sep=',')
...
\begin{figure}[H]
    ...
    p0_0.25_w0_0.25_m1a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.25_m1a_mles.csv",header=F)
    colnames(p0_0.25_w0_0.25_m1a_mles) <- c('k','p0','p1','w0','w1')
    p0_0.25_w0_0.5_m1a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_m1a_mles.csv",header=F)
    colnames(p0_0.25_w0_0.5_m1a_mles) <- c('k','p0','p1','w0','w1')
    p0_0.5_w0_0.25_m1a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.25_m1a_mles.csv",header=F)
    colnames(p0_0.5_w0_0.25_m1a_mles) <- c('k','p0','p1','w0','w1')
    p0_0.5_w0_0.5_m1a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.5_m1a_mles.csv",header=F)
    colnames(p0_0.5_w0_0.5_m1a_mles) <- c('k','p0','p1','w0','w1')
    p0_0.75_w0_0.25_m1a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.25_m1a_mles.csv",header=F)
    colnames(p0_0.75_w0_0.25_m1a_mles) <- c('k','p0','p1','w0','w1')
    p0_0.75_w0_0.5_m1a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.75_w0_0.5_m1a_mles.csv",header=F)
...
\begin{figure}[H]
    ...
    p0_0.25_w0_0.5_5_taxa_m0_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_m0_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_m1a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_m1a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_m2a_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_m2a_lnLs.csv",sep=',')
    p0_0.25_w0_0.5_5_taxa_c2_lnl <- scan("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_c2_lnLs.csv",sep=',')
...
\begin{figure}[H]
  \centering
<<klp0.75w0.5, echo=F>>=
site.lik.truth <- read.csv("../../sim/null/5_taxa_bl_3_10000_codons/data/sim_p0_0.75_w0_0.5/site_lik_truth.csv",header=F)
colnames(site.lik.truth) <- c('cnt','lik')
n <- sum(site.lik.truth$cnt)
cnt <- site.lik.truth$cnt
y.truth <- site.lik.truth$lik
...
for (p in seq(0,1,.1)) {
    for (w in seq(0,1,.1)) {
        site.lik <- read.csv(paste("../../sim/null/5_taxa_bl_3_10000_codons/data/sim_p0_0.75_w0_0.5/site_like_p_",p,"_w_",w,".csv",sep=''),header=F)
        y <- site.lik[,2]
        y[y==0] <- 1e-12
        y.bar <- sum(cnt*log(y.truth / y))/n
        kl <- c(kl,y.bar)
...
%       $\beta$-globin         & 48             & \\
%       \textit{ccmF}          & 27 193 195 199 &  \\
%       \textit{CDH3}          & \specialcell{14 21 30 44 46 48 56 76 85 374 427 488 507\\521 523 552 592 650 790} & \specialcell{59 79 458 465 561 562 585 614 645 646 649} \\
%       \textit{ENAM}          & \specialcell{8 102 109 110 150 168 190 285 290 317 327 336 340\\345 353 363 429 437 523 527 536 559 615 621 657\\660 664 680 728 752 838 850 854 880 922 923 941\\968 976 1008 1012 1069 1076 1116} & \specialcell{68 102 168 190 285 290 327 336 345 353 363 523 536 615\\664 838 922 923 1012 1069} \\
%       \textit{env}           & \specialcell{9 18 21 22 24 26 28 31 36 39 40 46\\54 57 59 66 68 69 73 75 76 83 84 87 90} & \specialcell{1 9 22 24 26 28 31 39 40 46 66 68 69 76 83 87}  \\
%       lysin                  & \specialcell{4 6 7 9 10 11 12 14 32 33 36 41 64 67 68 70\\74 75 79 80 83 86 87 100 113 115 116 119 120 123\\126 127 132 134} & \specialcell{4 6 7 9 10 11 12 32 33 36 44 64 70 74 75 79 83 86 87 100\\113 115 116 119 120 126 127 134} \\
%       \textit{mivN}          & & 1 217 340 392 442 476 492 \\
%       \textit{nuoL3}         & \specialcell{51 154 348 352 353 354 355 356 357 358 359 363 368\\370 373 378 380 453} & \specialcell{51 154 348 352 353 354 355 356 357 358 359 363 368 370\\373 378 380 453} \\
%       \textit{perM}          & 13 44 72 108 172 351 & 13 44 72 108 172 351 \\
%       \textit{pgpA}          & & 38 69 132 198 \\
%       \textit{pol}           & \specialcell{2 3 4 14 41 67 313 347 379 478 568 654 761 779} & \specialcell{2 3 4 14 41 67 313 347 379 388 431 459 462 478 568 654\\761 779 782} \\
%       \textit{RfaL}          & \specialcell{39 40 41 42 47 48 51 53 54 55 56 57 59 61 62 63 64\\66 67 68 69 112 118 251 297 352} & \specialcell{39 41 42 47 48 51 53 54 55 57 59 62 63 64 66 67 68 69\\112 352} \\
%       \textit{TrbL-VirB6\_2} &  & \specialcell{66 67 155 157 303 364 382 518 534 621 623 628 641 643\\650 653} \\
%       \textit{TrbL-VirB6\_3} & \specialcell{44 101 144 238 241 243 245 246 247 248 249 251 252\\256 259 261 267 272 273 275 289 291 295 302 323\\336 337 344 345 931 932} & \specialcell{101 144 238 241 245 246 247 248 249 251 252 256 259\\261 267 272 273 275 289 291 295 302 323 337 344 931\\932} \\
%       \textit{vif}           & \specialcell{31 33 37 39 47 63 92 101 109 110 122 127 128 167} & \specialcell{31 33 37 39 47 63 92 101 109 122 127 128 167} \\
%       \bottomrule
%     \end{tabular}
%     \label{tab:SitesIded}
%     \begin{tablenotes}
...
% \begin{figure}[H]
%     \centering
%     <<CDF32taxaLongBL, echo=F,warning=F>>=
%     p0_0.25_w0_0.5_32_taxa_m1a_lnl <- scan("../../sim/null/32_taxa_balanced_tree/data/p0_0.25_w0_0.5_m1a_lnLs_fixed_long_bl.csv",sep=',')
%     p0_0.25_w0_0.5_32_taxa_m2a_lnl <- scan("../../sim/null/32_taxa_balanced_tree/data/p0_0.25_w0_0.5_m2a_lnLs_fixed_long_bl.csv",sep=',')
%     p0_0.25_w0_0.5_32_taxa_c2_lnl <- scan("../../sim/null/32_taxa_balanced_tree/data/p0_0.25_w0_0.5_c2_lnLs_fixed_long_bl.csv",sep=',')

%     N <- length(p0_0.25_w0_0.5_32_taxa_m1a_lnl)
%     x <- seq(0,6.3,length.out=N)

%     lrs_p0_0.25_w0_0.5_32_taxa_m2a <- sort(2*(p0_0.25_w0_0.5_32_taxa_m2a_lnl-p0_0.25_w0_0.5_32_taxa_m1a_lnl))
%     lrs_p0_0.25_w0_0.5_32_taxa_c2 <- sort(2*(p0_0.25_w0_0.5_32_taxa_c2_lnl-p0_0.25_w0_0.5_32_taxa_m1a_lnl))

%     lrs <- c(x,lrs_p0_0.25_w0_0.5_32_taxa_m2a,lrs_p0_0.25_w0_0.5_32_taxa_c2)

%     prob.t <- 1/2+pchisq(x,1)/2
%     cprob <- c(prob.t,rep(1:N/N,2))
...
%               panel.border = element_rect(colour = "black", fill=NA, size=1))
% @
% \caption[]{CDF of LR statistics for data simulated with more taxa and longer branch lengths.  The LR statistics were calculated with and without likelihood modification under the nested model pair M1a/M2a for 1000 sequence alignments.  The alignments were simulated with 25\% of the sites evolving under $\omega=0.5$ and the remaining sites evolving under $\omega=1$ using a balanced 32-taxon tree topology with branch lengths summing to 9.  A modified likelihood tuning parameters of $C=2$ was used.  A curve for the theoretical CDF, an equal mixture of $\chi^2_0$ and $\chi^2_1$, is included.}
%   \label{fig:CDF32taxa9tl}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     <<CDFAlt, echo=F,warning=F>>=

%     p0_0.45_p1_0.45_p2_0.1_w2_1.5_m1a_lnl <- scan("../../sim/alt/5_taxa_balanced_tree/p0_0.45_p1_0.45_p2_0.1_w2_1.5_m1a_lnLs.csv",sep=',')
%     p0_0.45_p1_0.45_p2_0.1_w2_1.5_m2a_lnl <- scan("../../sim/alt/5_taxa_balanced_tree/p0_0.45_p1_0.45_p2_0.1_w2_1.5_m2a_lnLs.csv",sep=',')
%     p0_0.45_p1_0.45_p2_0.1_w2_1.5_c2_lnl <- scan("../../sim/alt/5_taxa_balanced_tree/p0_0.45_p1_0.45_p2_0.1_w2_1.5_c2_lnLs.csv",sep=',')

%     lrs_p0_0.45_p1_0.45_p2_0.1_w2_1.5_m2a <- sort(2*(p0_0.45_p1_0.45_p2_0.1_w2_1.5_m2a_lnl-p0_0.45_p1_0.45_p2_0.1_w2_1.5_m1a_lnl))
%     lrs_p0_0.45_p1_0.45_p2_0.1_w2_1.5_c2 <- sort(2*(p0_0.45_p1_0.45_p2_0.1_w2_1.5_c2_lnl-p0_0.45_p1_0.45_p2_0.1_w2_1.5_m1a_lnl))

%     N <- length(p0_0.45_p1_0.45_p2_0.1_w2_1.5_m1a_lnl)
%     x <- seq(0,15.3,length.out=N)

%     lrs <- c(x,lrs_p0_0.45_p1_0.45_p2_0.1_w2_1.5_m2a,lrs_p0_0.45_p1_0.45_p2_0.1_w2_1.5_c2)

%     cprob <- c(1/2+pchisq(x,1)/2,1:N/N,1:N/N)

%     cdf.data <- data.frame(lrs,cprob,model=rep(c('Theory','M2a (C=0)','M2a (C=2)'),each=N))

%     cdf.plot <- ggplot(cdf.data,aes(lrs,cprob)) +
%         coord_cartesian(xlim=c(0,11.5), ylim=c(0,1)) +
%         labs(x="LRS",y=expression("P(X"<="x)")) +
%         geom_line(aes(color=model,linetype=model),size=.5) +
%         scale_color_manual(values=c("black","black","black")) +
%         scale_linetype_manual(values=c('dotted','dashed','solid')) +
...
%               panel.border = element_rect(colour = "black", fill=NA, size=1))
% @
%   \caption[]{CDFs under the alternative. Theory (null)...}
%   \label{fig:CDFAlt}
% \end{figure}
...
% \begin{figure}[H]
%     \centering
%     <<p2M2aMLEs, echo=F,warning=F>>=
%     p0_0.25_w0_0.25_m1a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.25_m2a_mles.csv",header=F)
%     colnames(p0_0.25_w0_0.25_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.25_w0_0.5_m2a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_m2a_mles.csv",header=F)
%     colnames(p0_0.25_w0_0.5_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.5_w0_0.25_m2a_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.5_w0_0.25_m2a_mles.csv",header=F)
...
%     p2 <- c(p0_0.25_w0_0.25_m2a_mles$p2,p0_0.25_w0_0.5_m2a_mles$p2,
%                   p0_0.5_w0_0.25_m2a_mles$p2,p0_0.5_w0_0.5_m2a_mles$p2,
%                   p0_0.75_w0_0.25_m2a_mles$p2,p0_0.75_w0_0.5_m2a_mles$p2)

%     N <- nrow(p0_0.25_w0_0.25_m2a_mles)

%     mle.data <- data.frame(p2,
%                            weight=rep(c(0.25,0.5,0.75),each=2*N),
%                            omega=rep(c(0.25,0.5),each=N,times=3))

%     cdf.plot <- ggplot(mle.data,aes(p2,y=..ncount..)) +
%         geom_histogram(binwidth=.005,fill=I('black')) +
%         labs(x=expression(p[2]),y='') +
% facet_grid(weight~omega,labeller=label_bquote(cols=omega[0]*'='*.(omega),rows=p[0]*'='*.(weight)))
...
%               #panel.border = element_rect(colour = "black", fill=NA, size=0))
% @
% \caption[]{MLEs of the $p_2$ parameter under model M2a without likelihood modification.  For each simulation setting, 10,000 sequence alignments were generated with two site classes, $\omega<1$ and $\omega=1$ using a balanced, 5-taxon tree topology with branch lengths summing to 3.  The value of $\omega_0$ and its weight, $p_0$, used to simulate the data are shown as column and row labels.}
%   \label{fig:p2M2aMLEs}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     <<p2C2MLEs, echo=F,warning=F>>=
%     p0_0.25_w0_0.25_c2_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.25_c2_mles.csv",header=F)
%     colnames(p0_0.25_w0_0.25_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.25_w0_0.5_c2_mles <- read.csv("../../sim/null/5_taxa_bl_3/data/p0_0.25_w0_0.5_c2_mles.csv",header=F)
%     colnames(p0_0.25_w0_0.5_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
...
%     p2 <- c(p0_0.25_w0_0.25_c2_mles$p2,p0_0.25_w0_0.5_c2_mles$p2,
%                   p0_0.5_w0_0.25_c2_mles$p2,p0_0.5_w0_0.5_c2_mles$p2,
%                   p0_0.75_w0_0.25_c2_mles$p2,p0_0.75_w0_0.5_c2_mles$p2)
...
%               #panel.border = element_rect(colour = "black", fill=NA, size=0))
% @
% \caption[]{MLEs of the $p_2$ parameter under model M2a using a modified likelihood parameter of $C=2$ for six simulation settings.  For each simulation setting, 10,000 sequence alignments were generated with two site classes, $\omega<1$ and $\omega=1$ using a balanced, 5-taxon tree topology with branch lengths summing to 3.  The value of $\omega_0$ and its weight, $p_0$, used to simulate the data are shown as column and row labels.}
%   \label{fig:p2C2MLEs}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     <<p232taxaM2aMLEs, echo=F,warning=F>>=
%     rm(list=ls())
%     p0_0.25_w0_0.25_m2a_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.25_w0_0.25_m2a_mles.csv",header=F)
%     colnames(p0_0.25_w0_0.25_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.25_w0_0.5_m2a_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.25_w0_0.5_m2a_mles.csv",header=F)
%     colnames(p0_0.25_w0_0.5_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.5_w0_0.25_m2a_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.5_w0_0.25_m2a_mles.csv",header=F)
%     colnames(p0_0.5_w0_0.25_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.5_w0_0.5_m2a_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.5_w0_0.5_m2a_mles.csv",header=F)
%     colnames(p0_0.5_w0_0.5_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.75_w0_0.25_m2a_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.75_w0_0.25_m2a_mles.csv",header=F)
%     colnames(p0_0.75_w0_0.25_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.75_w0_0.5_m2a_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.75_w0_0.5_m2a_mles.csv",header=F)
%     colnames(p0_0.75_w0_0.5_m2a_mles) <- c('k','p0','p1','p2','w0','w1','w2')
...
%               #panel.border = element_rect(colour = "black", fill=NA, size=0))
% @
% \caption[]{MLEs of the $p_2$ parameter under model M2a without likelihood modification.  For each simulation setting, 1,000 sequence alignments were generated with two site classes, $\omega<1$ and $\omega=1$ using a balanced, 32-taxon tree topology with branch lengths summing to 3.  The value of $\omega_0$ and its weight, $p_0$, used to simulate the data are shown as column and row labels.}
%   \label{fig:p232taxaM2aMLEs}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     <<p232taxaC2MLEs, echo=F,warning=F>>=
%     rm(list=ls())
%     p0_0.25_w0_0.25_c2_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.25_w0_0.25_c2_mles.csv",header=F)
%     colnames(p0_0.25_w0_0.25_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.25_w0_0.5_c2_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.25_w0_0.5_c2_mles.csv",header=F)
%     colnames(p0_0.25_w0_0.5_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.5_w0_0.25_c2_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.5_w0_0.25_c2_mles.csv",header=F)
%     colnames(p0_0.5_w0_0.25_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.5_w0_0.5_c2_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.5_w0_0.5_c2_mles.csv",header=F)
%     colnames(p0_0.5_w0_0.5_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.75_w0_0.25_c2_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.75_w0_0.25_c2_mles.csv",header=F)
%     colnames(p0_0.75_w0_0.25_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
%     p0_0.75_w0_0.5_c2_mles <- read.csv("../../sim/null/32_taxa_balanced_tree/data/p0_0.75_w0_0.5_c2_mles.csv",header=F)
%     colnames(p0_0.75_w0_0.5_c2_mles) <- c('k','p0','p1','p2','w0','w1','w2')
